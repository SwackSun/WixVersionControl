...
CMFCSampleDlg::CMFCSampleDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MFC_SAMPLE_DIALOG, pParent)
{
...
BOOL CMFCSampleDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	CString ver = GetAppVersion();
	if (!ver.IsEmpty())
	{
		//int pos = ver.ReverseFind('.');
		//CString mainVer = ver.Left(pos);
		//CString build = ver.Right(ver.GetLength() - pos - 1);
		//GetDlgItem(IDC_STATIC)->SetWindowText(mainVer);
		GetDlgItem(IDC_STATIC)->SetWindowText(L"version:"+ver);
	}
...
CString CMFCSampleDlg::GetAppVersion()
{
